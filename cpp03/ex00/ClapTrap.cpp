...
/*   Updated: 2023/10/03 04:42:37 by zlaarous         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
ClapTrap::ClapTrap(string name): name(name) ,hit_points(10), energy_points(10), attack_points(0){
...
void ClapTrap::attack(std::string const &target)
{
    if (hit_points > 0 && energy_points > 0) {
        std::cout << "ClapTrap " << name << " attacks " << target << " for " << attack_points << " damage!" << std::endl;
        energy_points--;
    } else {
...
void ClapTrap::takeDamage(unsigned int amount)
{
    if (hit_points > 0) {
        if (amount >= hit_points) {
            ...
            hit_points -= amount;
            std::cout << "ClapTrap " << name << " takes " << amount << " damage. Hit points: " << hit_points << std::endl;
        }
    } else {
        std::cout << "ClapTrap " << name << " is already destroyed. No effect." << std::endl;
    }
}
...
void ClapTrap::beRepaired(unsigned int amount)
{
    if (hit_points == 0) {
        ...
        std::cout << "ClapTrap " << name << " is repaired for " << amount << " hit points. Hit points: " << hit_points << std::endl;
